...
import { fetchIntegratedTravelNoteSearchList } from "../../../apis/search";
...
const SearchPublicTravelNoteProfile: React.FC<{ content: string }> = (
  props
) => {
  const { content } = props;
  ...
  const { data, fetchNextPage, hasNextPage } =
    useInfiniteQuery<ITravelNoteProfileListType>(
      [queryKeys.searchPublicTravelNote, content, apiOption],
      ({ pageParam = 1 }) =>
        fetchIntegratedTravelNoteSearchList(content, pageParam, 16, apiOption),
      {
...
export default SearchPublicTravelNoteProfile;
